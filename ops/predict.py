...
df = pd.read_csv("/dagster_pipeline/Data/processed_data.csv")
with open("/dagster_pipeline/Data/models.pkl", "rb") as f:
...
def predict_senator(user_input_text):
    ...
    # Display results
    print(f"Overall Sentiment: {overall_sentiment_label} ({overall_sentiment_score:.2f})\n")
    print("Top 12 Senator Matches:")
    for senator, similarity in sorted_output:
        print(f"{senator[0]} {senator[1]}: {similarity:.2f}%")

# Example usage
if __name__ == "__main__":
    user_input_text = input("Enter your political comment: ")
    predict_senator(user_input_text)
